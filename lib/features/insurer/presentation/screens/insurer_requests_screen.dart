...
class InsurerRequestsScreen extends ConsumerWidget {
  const InsurerRequestsScreen({super.key});
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => ref.invalidate(insuredRequestsProvider),
          ),
        ],
      ),
      body: requestsAsync.when(
        loading: () => const Center(child: CircularProgressIndicator(color: _secondaryColor)),
        error: (e, _) => _ErrorState(onRetry: () => ref.invalidate(insuredRequestsProvider)),
            ...
            : RefreshIndicator(
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (context, i) => _RequestTile(
                    request: requests[i],
                    onTap: () => context.push('/insurer-request-detail/${requests[i].id}'),
                  ),
                ),
              ),
      ),
        ...
        child: Row(
          children: [
            Container(
              ...
              height: 44,
              decoration: BoxDecoration(
                color: _insuredColor.withValues(alpha: 0.1),
                shape: BoxShape.circle,
              ),
              child: const Icon(Icons.shield_outlined, color: _insuredColor, size: 22),
            ),
                    ...
                    children: [
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(color: _insuredColor.withValues(alpha: 0.25)),
                        ),
                        child: const Text(
                          'Certificado',
                          style: TextStyle(
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: const Icon(Icons.shield_outlined, color: _secondaryColor, size: 36),
            ),
            const SizedBox(height: 20),
            const Text(
              'Sin proyectos para asegurar',
              style: TextStyle(
                fontFamily: 'Manrope',
                fontSize: 20,
                fontWeight: FontWeight.w700,
            ...
            const Text(
              'Los proyectos certificados aparecerán aquí para la creación de planes de seguro.',
